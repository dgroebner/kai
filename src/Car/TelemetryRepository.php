        ...
        try {
            ...
            $stmtState = $this->db->prepare("
                INSERT INTO `vehicle_state` (
                    ...
                    `soc_percent`      = IF(VALUES(`soc_percent`) IS NULL, `soc_percent`, VALUES(`soc_percent`)),
                    `target_soc`       = IF(VALUES(`target_soc`) IS NULL, `target_soc`, VALUES(`target_soc`)),
                    `charge_power_kw`  = IF(VALUES(`charge_power_kw`) IS NULL, `charge_power_kw`, VALUES(`charge_power_kw`)),
                    `battery_temp_max` = IF(VALUES(`battery_temp_max`) IS NULL, `battery_temp_max`, VALUES(`battery_temp_max`)),
                    `battery_temp_min` = IF(VALUES(`battery_temp_min`) IS NULL, `battery_temp_min`, VALUES(`battery_temp_min`)),
                    `charging_state`   = IF(VALUES(`charging_state`) IS NULL, `charging_state`, VALUES(`charging_state`)),
                    `plug_connected`   = IF(VALUES(`plug_connected`) IS NULL, `plug_connected`, VALUES(`plug_connected`)),
                    `is_locked`        = IF(VALUES(`is_locked`) IS NULL, `is_locked`, VALUES(`is_locked`)),
                    `mileage_km`       = IF(VALUES(`mileage_km`) IS NULL, `mileage_km`, VALUES(`mileage_km`)),
                    `range_km`         = IF(VALUES(`range_km`) IS NULL, `range_km`, VALUES(`range_km`)),
                    `outdoor_temp_c`   = IF(VALUES(`outdoor_temp_c`) IS NULL, `outdoor_temp_c`, VALUES(`outdoor_temp_c`)),
